...
# If Anthropic key — no proxy needed, launch via wrapper
if ($env:ANTHROPIC_API_KEY) {
  Write-Host "  ✅  Anthropic API key detected — launching with Jelly (Claude backend)."
  Write-Host ""
...
if (-not $env:OPENROUTER_API_KEY) {
  Write-Host "  ℹ️  No API key found — falling through to Jelly login."
  Write-Host ""
